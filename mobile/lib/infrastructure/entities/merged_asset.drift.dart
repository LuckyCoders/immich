...
class MergedAssetDrift extends i1.ModularAccessor {
  ...
  i0.Selectable<MergedBucketResult> mergedBucket({
    ...
    $arrayStartIndex += userIds.length;
    return customSelect(
      'SELECT COUNT(*) AS asset_count, bucket_date FROM (SELECT CASE WHEN ?1 = 0 THEN COALESCE(STRFTIME(\'%Y-%m-%d\', rae.local_date_time), STRFTIME(\'%Y-%m-%d\', rae.created_at, \'localtime\'), \'1970-01-01\') WHEN ?1 = 1 THEN COALESCE(STRFTIME(\'%Y-%m\', rae.local_date_time), STRFTIME(\'%Y-%m\', rae.created_at, \'localtime\'), \'1970-01\') ELSE COALESCE(STRFTIME(\'%Y-%m-%d\', rae.local_date_time), STRFTIME(\'%Y-%m-%d\', rae.created_at, \'localtime\'), \'1970-01-01\') END AS bucket_date FROM remote_asset_entity AS rae LEFT JOIN stack_entity AS se ON rae.stack_id = se.id WHERE rae.deleted_at IS NULL AND rae.visibility = 0 AND rae.owner_id IN ($expandeduserIds) AND(rae.stack_id IS NULL OR rae.id = se.primary_asset_id)UNION ALL SELECT CASE WHEN ?1 = 0 THEN COALESCE(STRFTIME(\'%Y-%m-%d\', lae.created_at, \'localtime\'), STRFTIME(\'%Y-%m-%d\', lae.updated_at, \'localtime\'), \'1970-01-01\') WHEN ?1 = 1 THEN COALESCE(STRFTIME(\'%Y-%m\', lae.created_at, \'localtime\'), STRFTIME(\'%Y-%m\', lae.updated_at, \'localtime\'), \'1970-01\') ELSE COALESCE(STRFTIME(\'%Y-%m-%d\', lae.created_at, \'localtime\'), STRFTIME(\'%Y-%m-%d\', lae.updated_at, \'localtime\'), \'1970-01-01\') END AS bucket_date FROM local_asset_entity AS lae WHERE NOT EXISTS (SELECT 1 FROM remote_asset_entity AS rae WHERE rae.checksum = lae.checksum AND rae.owner_id IN ($expandeduserIds)) AND EXISTS (SELECT 1 FROM local_album_asset_entity AS laa INNER JOIN local_album_entity AS la ON laa.album_id = la.id WHERE laa.asset_id = lae.id AND la.backup_selection = 0) AND NOT EXISTS (SELECT 1 FROM local_album_asset_entity AS laa INNER JOIN local_album_entity AS la ON laa.album_id = la.id WHERE laa.asset_id = lae.id AND la.backup_selection = 2)) GROUP BY bucket_date ORDER BY bucket_date DESC',
      variables: [
